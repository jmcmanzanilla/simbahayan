...
<?php
	...
	if(isset($_POST['submitted'])) {
		...
		$query = "SELECT * FROM proj_proposal";
		$conditions = array();

    if($status !="") {
                $sql = "SELECT * FROM proj_proposal WHERE status = '$status' AND lead_org = '$user_id'";
...
$sql="";
	...
	while($row = mysqli_fetch_array($result)) {
					...
					echo "<th> <a href='org-viewproposal.php?d=$row[proposal_id]'>View</a></th>";
					echo "</tr>";
	}
	echo "</tbody>";
		echo "</table>";
