...
for ($x=0; $x < 3; $x++) {
	...
	$query = "SELECT * FROM order_signatory WHERE org_num = '". $d."' AND standard = 0";  
	...
	$sqlinsert = "INSERT INTO order_signatory (order_number, signatory_num, org_num, signatory_name, reference) VALUES ('$order_number','$signatory_num', '$org_num','$signatory_name', '$id')";
	
	if(!mysqli_query($link, $sqlinsert)){
		die("FAILED SUBMISSION");
	}
	echo "<script type='text/javascript'>alert('SUCCESSFULLY ADDED ACCOUNT'); window.location = 'manage-order1.php?d=$d';</script>";
}
  ...
  <ul class="nav navbar-nav navbar-right" style = "margin-top: 30px;">
    ...
    <div class = "container">
            ...
            <div class = "col-md-12">
                ...
                <div class = "panel panel-body" style = "width: 100%; border-radius: 0;height: 500px; background-color: #fff; color: #000; box-shadow: 5px 5px 5px grey;">
             ...
             <?php  
                echo "<form role='form' class='col-md-5' action = 'manage-order1.php?d=$d' method = 'post' enctype='multipart/form-data'>
        ...
        <h3 style="margin-top:0px;">CREATE SIGNATORY ORDER </h3><br>
             <div class="col-md-12">
				...
				<div class="col-md-5">
                    ...
                    <?php
						$query = mysqli_query($link, "SELECT * from login_user WHERE account_type = 'Signatory' AND reference = $id OR user_id = $id");
				...
				<div class="form-group col-md-7" style="margin-top:10px;">
					<input type="hidden" name="order_number[2]" value="3"/>
					<h5>Coordinator</h5>
                </div>
				...
				<div class="form-group col-md-5" style="margin-top:10px;">
                    <?php
						$query = mysqli_query($link, "SELECT * from login_user WHERE account_type = 'Signatory' AND reference = $id OR user_id = $id");
						echo '<select  name="signatory[2]" class="form-control" required >';
						echo '<option value=""> </option>';
						while ($row = mysqli_fetch_array($query)) {
							echo '<option value="'.$row['user_id'].'">'.$row['name'].'</option>';
						}
				...
				<div class="form-group col-md-7" style="margin-top:10px;">
					<input type="hidden" name="order_number[3]" value="4"/>
					<h5>Department Chairman (optional) </h5>
                </div>
            	
				<div class="form-group col-md-5" style="margin-top:10px;">
                    <?php
						$query = mysqli_query($link, "SELECT * from login_user WHERE account_type = 'Signatory' AND reference = $id");
						echo '<select  name="signatory[3]" class="form-control">';
						echo '<option value=""> </option>';
						while ($row = mysqli_fetch_array($query)) {
							echo '<option value="'.$row['user_id'].'">'.$row['name'].'</option>';
						}
