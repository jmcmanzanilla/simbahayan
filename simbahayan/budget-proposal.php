...
if(isset($_POST['submitted'])){
   ...
   //$totalcom = $totalcom + ($ptotalcom + $mtotalcom + $ototalcom + $ottotalcom);
	...
	if (!mysqli_query($link, $sqlprogram)){
	...
	echo "<script type='text/javascript'>alert('SUCCESSFULLY ADDED PROGRAM FLOW AND BUDGET PROPOSAL'); window.location ='org-viewproposal.php?d=$d';</script>";
...
<?php
	    ...
	    <input type="checkbox" name="vehicle" value="Bus 3 (70 seater)">Bus 3 (70 seater)
          ...
          <tr id="opslog5">
            <td>6</td>
                      <td><input type="text" name="oname5['.$x.']" placeholder="Fuel" class="form-control"/></td>
